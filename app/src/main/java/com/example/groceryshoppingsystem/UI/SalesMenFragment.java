...
import com.example.groceryshoppingsystem.Adapters.AdminCatogryAdapter;
...
public class SalesMenFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mDataBaseRef = FirebaseDatabase.getInstance("https://grocery-delivery-app-22f4e-default-rtdb.firebaseio.com/").getReference().child("GrocaryApp").child("categories");
            ...
            public void onItemLongClick(final int pos) {
                AlertDialog.Builder dialog = new AlertDialog.Builder(getActivity()).setTitle("Confirmation").setMessage("Are You Sure You Want To Delete ?!").setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        ...
                        StorageReference a = FirebaseStorage.getInstance("gs://grocery-delivery-app-22f4e.appspot.com").getReference().child("GrocaryApp").child("categories").child(adminCatogryList.get(pos).getName() + ".jpg");
                        a.delete();
                        StorageReference b = FirebaseStorage.getInstance("gs://grocery-delivery-app-22f4e.appspot.com").getReference().child("GrocaryApp").child("categories").child(adminCatogryList.get(pos).getName());
                        b.delete();
                        StorageReference c = FirebaseStorage.getInstance("gs://grocery-delivery-app-22f4e.appspot.com").getReference().child("GrocaryApp").child("categories").child(adminCatogryList.get(pos).getName() + "qr.jpg");
                        c.delete();
                        StorageReference d = FirebaseStorage.getInstance("gs://grocery-delivery-app-22f4e.appspot.com").getReference().child("GrocaryApp").child("categories").child(adminCatogryList.get(pos).getName() + "qr");
                        d.delete();
                    }
